...
import { FaqAccordion, type FaqEntry } from "@/components/consultancy/FaqAccordion";

export function AdmissionConsultancyClient({ faqs }: { faqs: FaqEntry[] }) {
  const { recordLead, courseOptions, counsellors } = useAppState();
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-[1080px] px-[22px] py-12">
        <h2 className="mb-8 text-2xl font-extrabold text-primary-900">How it works</h2>
        <div className="relative grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))" }}>
          <div
            aria-hidden
            className="absolute top-[18px] right-[8%] left-[8%] hidden h-0.5 bg-[#E5E7EB] md:block"
            style={{ zIndex: 0 }}
          />
          {processFull.map((p) => (
            <div key={p.n} className="relative z-[1] rounded-[14px] border border-[#E5E7EB] bg-white p-5">
              <div className="mb-3.5 flex h-9 w-9 items-center justify-center rounded-full border-4 border-[#F7F6F2] bg-primary-900 font-extrabold text-highlight-500">
                {p.n}
              </div>
